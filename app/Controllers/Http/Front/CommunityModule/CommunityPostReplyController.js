...
class CommunityPostReplyController {
  ...
  async index({ request, response, view, auth }) {
    ...
    query.with("postReplyVote", (builder) => {
      builder
        .select("id", "community_post_reply_id")
    ...
    try {
      ...
      try {
        ...
        if (request.input("parent_id") > 0) {
          ...
          link = `${Env.get(
            "ADMIN_BASE_URL"
          )}/community-posts-reply-comments-form/${ins_community_post_reply_id}`;
        } else {
          ...
          link = `${Env.get(
            "ADMIN_BASE_URL"
          )}/community-posts-reply-form/${ins_community_post_reply_id}`;
        }
      ...
      console.log(error);
      Logger.transport("file").info(`error save answer/comment : ${error}`);
      trx.rollback();
      return response
        .status(423)
  ...
  async voting({ params, request, response, auth }) {
    ...
    try {
      ...
      const getData = await CommunityPostReply.query()
        .with("communityPost")
        .where("id", community_post_reply_id)
        .first();
      ...
      if (getData.visitor_id == userId) {
        return response
          .status(422)
          .send([{ message: "You can not vote on your own answer." }]);
      }
  ...
  async mark_correct_answer({ params, request, response, auth }) {
    ...
    const trx = await Database.beginTransaction();
    try {
      const updateData = await CommunityPostReply.findOrFail(
        community_post_reply_id
      );

      if (userId != updateData.visitor_id) {
        return response.status(423).json({
          message: "You have not permission to mark as correct answer.",
        });
      }

      const checkData = await CommunityPostReply.query()
        .where("community_post_id", updateData.community_post_id)
        .where("is_correct_answer", 1)
        .first();
      if (checkData) {
        return response
          .status(423)
          .json({ message: "The correct answer has already been found." });
      }
  ...
  async update({ params, request, response }) {
    ...
    try {
      ...
      console.log(error);
      Logger.transport("file").info(`error update answer/comment : ${error}`);
