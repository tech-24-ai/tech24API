...
class VendorCategoryController {
  async index({ request, response, view }) {
    const query = VendorCategory.query();
  ...
  async store({ request, response }) {
    const query = new VendorCategory();
  ...
  async destroy({ params, request, response }) {
    const query = await VendorCategory.findOrFail(params.id);
    try {
      await query.delete();
  ...
  async modules() {
    return Database.table("vendor_categories");
  }
}
